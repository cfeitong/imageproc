...
impl Filter for Sobel {
    ...
    where
        P: Pixel + Saturating {
        let _kernx = [-1f32, 0f32, 1f32,
                      -2f32, 0f32, 2f32,
                      -1f32, 0f32, 1f32];
        let _kerny= [-1f32, -2f32, -1f32,
                      0f32,  0f32,  0f32,
                      1f32,  2f32,  1f32];
        ...
        use std::fs::File;
        use std::io::Write;
        let mut file = File::create("/tmp/sobel-imgx.txt").unwrap();
        write!(file, "{:?}", imgx);
        let mut file = File::create("/tmp/sobel-imgy.txt").unwrap();
        write!(file, "{:?}", imgy);
